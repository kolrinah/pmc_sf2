...
class BannerController extends Controller
{
    ...
    {       
       ...
       // $defaultData = array();
        ...
        $data['bannerAdmin'] = $this->get('serviciosComunes')
                                    ->paginador($query, $firstResult);
        
        // EXAMINAMOS SI LA PETICION VIENE DE AJAX
        if (!($this->getRequest()->isXmlHttpRequest()))  
            return $this->render('PmcIntranetBundle:Banner:adminBanners.html.twig',
                    array('formulario' => $formulario->createView(), 
                                'data' => $data ));
        else
            return $this->render('PmcIntranetBundle:Banner:banners.html.twig',
                                    array('data' => $data ));
    }
    ...
    public function activarBannerAction(Request $request)
    {
       ...
       {  
           ...
           $data['action'] = 'UPDATE';
           $data['module'] = 'Banner';
           $data['description'] = 'Ativação do banner id: '.$banner->getId().
                                 ', "'.substr($banner->getNome(), 0, 20).'..."';
    ...
    public function actualizaOrdenAction(Request $request)
    {
       ...
       {  
           ...
           $data['action'] = 'UPDATE';
           $data['module'] = 'Banner';
           $data['description'] = 'Ordenação dos banners.';
           $this->get('serviciosComunes')->cadastrarLog($data);
   ...
   /*
    ...
    public function excluirBannerAction(Request $request)
    {
       ...
       if ($request->isMethod('POST')) 
       {  
           $id = $request->request->get('idBanner');           
           ...
           $clon = $banner->getNome(); //Clonamos titulo antes de eliminarlo
           ...
           $data['action'] = 'DELETE';
           $data['module'] = 'Avisos';
           $data['description'] = 'Exclusão do Banner id: '.$id.
                                 ', "'.substr($clon, 0, 20).'..."';
           $this->get('serviciosComunes')->cadastrarLog($data);
    ...
    public function novoBannerAction(Request $request)
    {
       ...
       {   
           ...
           {  // Si hay imagen         
              ...
              if (!$error)
              {
                ...
                $data['action'] = 'INSERT';
                $data['module'] = 'Banner';
                $data['description'] = 'Novo Banner id: '.$banner->getId().
                                       ', "'.substr($banner->getNome(), 0, 20).'..."';
                $this->get('serviciosComunes')->cadastrarLog($data); 
       ...
       return $this->render('PmcIntranetBundle:Banner:crudBanner.html.twig',array(
       ...
       {        
           ...
           {           
              ...
              if (!$error)
              {
                  ...
                  $data['action'] = 'UPDATE';
                  $data['module'] = 'Banner';
                  $data['description'] = 'Edição do Banner id: '.$banner->getId().
                                         ', "'.substr($banner->getNome(), 0, 20).'..."';
                  $this->get('serviciosComunes')->cadastrarLog($data);         
                  
                  // ACTUALIZACIÓN SATISFACTORIA REDIRECCIONAMOS A BANNERS
                  $this->get('session')->getFlashBag()
                                       ->add('info', 'o Banner foi atualizado com sucesso');
                  return $this->redirect($this->generateUrl('adminBanners'));  
              }  
           }   
       }     
